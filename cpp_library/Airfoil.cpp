...
namespace nde {
	...
	Vector<Panel2D > Airfoil::getPanels(double density) const {
		...
		double chord = naca_airfoil.getChord();
		int num_panels = floor(1.0 / density);
		double dx = chord / double(num_panels);
		...
		for (int i = 0; i < num_panels; ++i) {
			...
			p_start_bottom(0) = density * (num_panels - i) * chord;
			p_start_bottom(1) = naca_airfoil.bottom(p_start_bottom(0));
			p_end_bottom(0) = density * (num_panels - i - 1) * chord;
			...
			p_start_top(0) = density * i * chord;
			p_start_top(1) = naca_airfoil.top(p_start_top(0));
			p_end_top(0) = density * (i + 1) * chord;
